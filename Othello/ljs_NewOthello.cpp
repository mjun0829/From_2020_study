    ...
    for (int i = Size-1; i >= 0;i--) {
        ...
        for (int j = 0; j < Size; j++) {
            if (Board[i][j].GetStatus() == EMPTY) {
                cout << "   ";
            } else if (Board[j][i].GetStatus() == WHITE) {
                cout << "●  ";
...
bool BoardManager::CheckColor(int NowX, int NowY, int JumpX, int JumpY, int Color){
...
bool BoardManager::IsOverSize(int NowX, int NowY, int JumpX, int JumpY){
  return NowX+JumpX>=Size || NowX+JumpX <0 || NowY+JumpY >=Size || NowY+JumpY <0;
}

bool BoardManager::FindMostCloseColor(int NowX, int NowY, int JumpX, int JumpY, int Color){
...
void BoardManager::ReverseBlocks(int X,int Y, int WayX, int WayY, int Color,vector<vector<Block>> Board){
    int CursorX = X + WayX;
    int CursorY = Y + WayY;
    while (true) {
