...
void ai_search(int greedy[8][8], int **mat, int y, int x, int y_dir, int x_dir,
  ...
  while (1) {
    ...
    if (mat[y_now + y_dir][x_now + x_dir] == opposite_turn(turn)) {
      greed++;
    }
    if (mat[y_now + y_dir][x_now + x_dir] == turn) {
      break;
    }
    y_now = y_now + y_dir;
    x_now = x_now + x_dir;
...
void ai_next_search(int **mat, int greedy[8][8], int row, int col, int a, int b,
                    int turn) {
  int temp_greedy[8][8] = {
      0,
  };
